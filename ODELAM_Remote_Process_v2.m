...
function ODELAM_Remote_Process_v2
%  Needs  Timer to check files
%  Load current state of image tracking.  Combined with ODELAYSave Data format;
%  Additional fields to SaveData.
%  Breakup Track and Next Field to do data analysis.

global OdelayFig

OdelayFig.checkTimer = timer('BusyMode', 'drop',...
                    'ExecutionMode', 'fixedRate',...
                    'Period', 3,...
                    'TimerFcn', @checkNewFiles,...
                    'TasksToExecute', Inf);

OdelayFig.hFig = figure('Position', [200,200,300,200],...
                    'ToolBar','none',...
                    'Name','ODELAM! Monitor',...
                    'DeleteFcn', @guiDelete);
                
OdelayFig.hAxes = axes('Units','pixels','Position', [25,80,250,100]);
OdelayFig.hImage = imagesc(zeros(96,96));
set(OdelayFig.hAxes,'XTick',[],'YTick',[],'XTickLabel',[],'YTickLabel',[]);

OdelayFig.hButton1 = uicontrol('Style', 'togglebutton',...
                    'Position',[10,10,100,50],...
                    'String','ODELAY!',...
                    'Callback',@guiStartMonitor);
                
OdelayFig.hButton2 = uicontrol('Style', 'togglebutton',...
                    'Position',[190,10,100,50],...
                    'String','Fit Curves!',...
                    'Callback',@guiFitCurves);
                
end

function guiDelete(hFig, events)
global monData OdelayFig

timerState = get(OdelayFig.checkTimer,'Running');
if ~strcmp(timerState,'off')
    stop(OdelayFig.checkTimer);
end
    delete(OdelayFig.checkTimer);

end

function guiStartMonitor(hFig, events)
global monData mP OdelayFig
%    monData = struct([]);
%     [monData(1).stageFile, monData(1).expDir, ~] = uigetfile('ODELAY_StageData.mat','Load Stage File');
%     cd(monData(1).expDir); 
%     load(fullfile(monData(1).expDir,monData(1).stageFile));
%     load(monData.IndexFile, 'monData');
    if isempty(monData)
    ...
    timerState = get(OdelayFig.checkTimer,'Running');
    if strcmp(timerState, 'off')
        set(OdelayFig.hButton1, 'String', 'Starting');
        start(OdelayFig.checkTimer);
        set(OdelayFig.hButton1, 'String', 'Running');
    else
        set(OdelayFig.hButton1, 'String', 'Stopping!');
        stop(OdelayFig.checkTimer);
        set(OdelayFig.hButton1, 'String', 'ODELAY!');
    end
...
function guiFitCurves(hFig, events)
global monData mP OdelayFig

    if isempty(monData)
         [monData, mP] = InitializeExperiment;
    end
numWells = monData.numWells;
monData.AnalyzedImages = false(numWells,240);
tempLedger = zeros(numWells, size(monData.AnalyzedImages,2));
wellInd = 1:numWells;
parfor well = 1:numel(wellInd)
%     well=63;
    try
        tempLedger(well,:) = WellProcess(wellInd(well), monData);
    catch
        tempLedger(well,:) = zeros(1,240);
    end
 end

end

function checkNewFiles(hTimer, events)
%% Timer Function to evaluate new files
% Look for new files:  check time stamp since last read
global monData mP OdelayFig

wellVec = 1:size(monData.AnalyzedImages,1);
cd(monData.expDir);

monitorFile = ls('ODELAY_Monitor.mat');

if ~isfield(monData, 'lastImaged')
    monData.lastImaged = zeros(size(mP.lastImaged));
    set(OdelayFig.hImage, 'CData',monData.AnalyzedImages);
end

if ~isempty(monitorFile) 
    numTry = 0;
    success = false;
    while numTry<=10 & success == false
        try
            data = load(monitorFile, 'lastImaged');
            success = true;
        catch
            pause(0.5);
            numTry = numTry+1;
    ...
    tfWells = data.lastImaged>monData.lastImaged;
    wellsIDed = mP.wellID(tfWells);
    wellRecIdx = 1:numel(monData.wellDir);
    wellLocIdx = 1:numel(mP.lastImaged);
    
    if ~isempty(wellsIDed)
%         fprintf(wellsIDed);
        analyzeWell = wellRecIdx(contains(monData.wellDir, wellsIDed));
        tempLedger = zeros(numel(analyzeWell), size(monData.AnalyzedImages,2));
%         analyzeWell = analyzeWell(1);

        if numel(analyzeWell>1)
            parfor w = 1:numel(analyzeWell)
               tempLedger(w,:) = WellProcess(analyzeWell(w), monData);
            end
           
            for w = 1:numel(analyzeWell)
                monData.AnalyzedImages(analyzeWell(w),:) = tempLedger(w,:);
            end

        else
             monData.AnalyzedImages(analyzeWell,:) =  WellProcess(analyzeWell, monData);
 
        end

        set(OdelayFig.hImage, 'CData',monData.AnalyzedImages);
        wellAnalyzed = wellLocIdx(contains(mP.wellID, wellsIDed));
        
        numTry = 0;
        success = false;
        while numTry<=10 & success == false
            try
                monData.lastImaged(wellAnalyzed) = data.lastImaged(wellAnalyzed);
                save(monData.IndexFile, 'monData', '-append');
                success = true;
            catch
                pause(0.5);
                numTry = numTry+1;
...
% Also Check to see if several images and wells need to be processed. If so
% spool up a multiple workers to get all the wells until it's caught up.
end
...
for aI = analyzIndex
    ...
    [RawImage, StitchMeta, RawStack,  fluorIm, fT1] = ODELAM_MatReadv5(imageFile, 20, monData.background);
...
% Check for douplacates in the objectNext column array and remove them
   ...
   if aI == 2
    ...
    nextList = nextInds(~isnan(objectTrack(:,aI-1)));
%     [maxTracks,maxInd] = max(objectNext(:,aI-1));
    if ~isempty(nextList)
...
for imNum =  2:numImage
    ...
    while prevImage~=1 & cnt < numImage
        ...
        revOrder(cnt) = row(1); % this is the reverse order of the path and will be flipped
    end
...
function prmsAll = gompertzFitParam(tdata, iareas, initVals)
...
% Restrict Fitting to colonies that at least doubled their size.  
if(areaMax>factor*iareas(1))
    
    % Use course grid optimization function findPrmsGompF to find 
    % a local minima based on the 
    if isempty(initVals) | isnan(initVals(1))        
       b=linspace(0.1,20,Nsteps);
        vecTlag = linspace(0,24*5*60,Nsteps);
        vecVmax = linspace(log(2)/(4*24*60),log(2)/60,Nsteps);

    % fit the first derivative of the gompertz function
        smoothArea = smoothdata(idata, 'movmean',5);
        [amin1, bmin1, estVmax, estTlag, fssq1] = findPrmsGompF1std(b, vecVmax, vecTlag, tdata, smoothArea);
        prms0 = [amin1 bmin1 estVmax estTlag];
   
    else
        prms0 = initVals(1:4);
    end
    % Use fmincon to find the local minima for the idata solutions
    meanArea = mean(idata(1:5));
    stdArea  = std(idata(1:5),0,2);
    
    aLow = meanArea-2*stdArea;
    aUp  = meanArea+2*stdArea;
    bLow = 0.1;
    bUp  = 20;
    velLow = 0;
    velUp  = 1;
    ...
    lb    = [aLow   bLow  velLow   lagLow];
    ub    = [aUp    bUp   velUp    lagUp];

    gompFunc = @(prms)gompFparam(prms, tdata, idata);
    options = optimoptions('fmincon','Algorithm','interior-point',...
                           'Display','off');
try                       
    [prms, fssq, exitflag] = fmincon(gompFunc,prms0,[],[],[],[],lb,ub,[],options);
catch
    test  = 1
end
    a    = prms(1);
    ...
    ATex     = gompFunction(prms, Tex);
    Aplateau = gompFunction(prms,1e50);
    prmsAll=[prms fssq Tlag Td Tex ATex Aplateau TdFlag TexFlag TVmax Tplat exitflag fssq/numtimepoints]; 
                
else
    prmsAll=[NaN NaN NaN NaN NaN NaN NaN NaN NaN NaN NaN NaN NaN NaN NaN NaN];
end    
  
end

function [amin, bmin, estVmax, estTlag, fmin ] = findPrmsGompF1std( b, vmax, tlag, tdata, idata )
%% findPrmsGompF calculates an initial estimate of the Gompertz function using a coarse grid optimization.

flag=0;
ditdata = diff(tdata,1,2);
didata = diff(idata,1,2)./ditdata;  % find numerical derivative with respect to time.
ctdata = tdata(1:end-1)+ditdata;    % correct time data vector to match derivative vector.
K = log((3+sqrt(5))/2);
for i=1:numel(b)
    for ii=1:numel(vmax)
        for iii=1:numel(tlag)
            yn = exp(1).*vmax(ii).*exp((vmax(ii)*exp(1)/b(i)).*(tlag(iii)-ctdata)+K).*exp(-exp((vmax(ii)*exp(1)/b(i)).*(tlag(iii)-ctdata)+K));
            ifunc = nansum((didata-yn).^2);
            if(flag==0||(flag==1&&ifunc<fmin));
                fmin    = ifunc;
                amin    = idata(1)-b(i)*exp(-exp((vmax(ii)*exp(1)/b(i))*tlag(iii)+K));
                bmin    = b(i);
                estVmax = vmax(ii);
                estTlag = tlag(iii);
                flag=1;
...
function f = gompFparam(x, tdata, idata)
...
yn=a+b.*exp(-exp((vmax*exp(1)/b).*(tlag-tdata)+log((3+sqrt(5))/2)));
f=nansum((yn-idata).^2);
         
end

function yn = gompFunction(x, tdata)
%% gompFparam calculates least squares distance of the paramerized gompertz function.
%
% from the logarithm of Gompertz-approximated growth curve values data through time.
%
...
function prmsAll = gompertzFitTvTLag(tData, iareas, initVals)
%% gompertzFitParam calculates an initial estimate of the Parameterized 
%% Gompertz Function 
% using a coarse grid optimization and then attempts to find 
% a constrained minimum of the Gompertz function at this initial estimate 
...
%
    ...
    y = idata([1,numtimepoints]);
    m = diff(y)/diff(x);
    yVals = m*tdata + y(1)-m*x(1);
    diffVals = idata-yVals;
   ...
   if  lagInd < vmxInd && vmxInd < texInd
        ...
        estVmax  = (idata(texInd)-idata(lagInd))/(tdata(texInd)-tdata(lagInd)); 
        estDT = tdata(vmxInd)-tdata(lagInd);
   elseif lagInd<vmxInd && (texInd<lagInd | texInd<vmxInd)
       estTlag  = tdata(lagInd);
       estVmax  = (idata(vmxInd)-idata(lagInd))/(tdata(vmxInd)-tdata(lagInd)); 
       estDT = tdata(vmxInd)-tdata(lagInd);
   ...
   elseif lagInd<texInd && (vmxInd<lagInd | vmxInd<texInd)
    ...
    % Use course grid optimization function findPrmsGompF to find 
    % a local minima based on the  
        dT=linspace(1,100*60,Nsteps);
        vmin = 200*60;
        vmax = 60;
        vecTlag = linspace(1,tdata(end),Nsteps);
        vecVmax = log(2)./linspace(vmin,vmax,Nsteps);
        smthArea = smoothdata(idata,'movmean',5);
        [estDT, estVmax, estTlag, fssq1] = findPrmsGompTDTF(dT, vecVmax, vecTlag, tdata,idata);
    ...
    stdArea  = std(idata(1:5),0,2);
    Klag = log((3+sqrt(5))/2);
    prms0 = [meanArea estDT estVmax estTlag];
    meanArea = mean(idata(1:5));
    stdArea  = std(idata(1:5),0,2);
    Klag = log((3+sqrt(5))/2);
    ...
    lb    = [aLow   dTLow  velLow   lagLow];
    ub    = [aUp    dTUp   velUp    lagUp];
    gompFunc = @(prms)gompParTvTl(prms, tdata, idata);
    options = optimoptions('fmincon','Algorithm','interior-point',...
                           'Display','off');
    ...
    ATex     = gompTvTl(prms, Tex);
    Aplateau = gompTvTl(prms,1e50);
    prmsAll=[prms fssq Tlag Td Tex ATex Aplateau TdFlag TexFlag TVmax Tplat exitflag fssq/numtimepoints]; 
         
end
 
function [dTmin, estVmax, estTlag, fmin ] = findPrmsGompTDTF( vecDT, vecVmax, vecTlag, tdata, idata )
%% findPrmsGompF calculates an initial estimate of the Gompertz function using a coarse grid optimization.
%
% Author: Alexander V. Ratushny
% Institute for Systems Biology
...
% October 2010; Last revision: 2016/03/01
   ...
   for vmax = vecVmax
       for tp = tVec(1:end-1)
           ...
           for dT = vecDt
                yn=a+(exp(1)*vmax*dT/K).*exp(-exp((K/dT).*(dT+tlag-tdata)));
...
function f = gompParTvTl(x, tdata, idata)
%% gompFparam calculates least squares distance of the paramerized gompertz function.
%
% from the logarithm of Gompertz-approximated growth curve values data through time.
%
...
cd(monData.saveDir);
fileList = ls('*ODELAYData.mat');
ExData = load(fileList(well,:));
...
ExperimentName = monData.expDir(stInd(end-1)+1:end-1);
...
ODELAY_StageData = ls('ODELAY_StageData.mat');

% if exist(ODELAY_StageData, 'file')==2
%    load(ODELAY_StageData);
%    monData(1).stageFile = fullfile(cd,ODELAY_StageData);
%    monData(1).expDir = cd;
% else
...
if isdir('ODELAY Well Data')
    monData.saveDir = fullfile(cd, 'ODELAY Well Data');
elseif ~isdir('ODELAY Well Data')
    mkdir('ODELAY Well Data');
    monData.saveDir = fullfile(cd, 'ODELAY Well Data');
end
...
monData.IndexFile = ls('*Index_ODELAYData.mat');
if isempty(monData.IndexFile)
    
    % Check for Experiment Description  
    ExpDiscriptionFile = ls('*ODELAYExpDisc.xlsx');
%     if ~isempty(ExpDiscriptionFile)
%         [~, ~, ~,ImageVarsTemp] = ODELAY_importExpDescription(ExpDiscriptionFile); 
%     else
    ...
    ImageVars.Magnification = 20;% mP.mag;
    ImageVars.PixSize = 6.45;...mP.pixSize;
