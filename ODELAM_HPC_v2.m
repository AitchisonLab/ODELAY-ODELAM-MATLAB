function ODELAM_HPC_v2(varargin)
...
for aI = analyzIndex
    ...
    % load New Image
    [RawImage, StitchMeta, RawStack,  fluorIm, fT1] = ODELAM_MatReadv5(imageFile, monData.magnifictation, monData.background);
...
%         oD.WellOut.prevImageStats = ImageStats;
       fNames = fieldnames(StitchMeta);
        ...
        oD.WellOut.xyDisp = NaN(size(oD.WellOut.ObjectNext,2),4);
    ...
    %% Centroid Association
        ...
        % in this way the ObjectNext is the row index of the next image timepoint
         ...
         for n = 1:16
             sw1 = zeros(imDim(1) + rDisp(n,3) , imDim(2) + cDisp(n,3));
             sw2 = zeros(imDim(1) + rDisp(n,3) , imDim(2) + cDisp(n,3));
             swT = zeros(imDim(1) + rDisp(n,3) , imDim(2) + cDisp(n,3));
             rv1 = rowVec + rDisp(n,1);
             cv1 = colVec + cDisp(n,1);
             rv2 = rowVec + rDisp(n,2);
             cv2 = colVec + cDisp(n,2);
             sw1(rv1, cv1) = bw1;
             sw2(rv2, cv2) = bw2;
             swT = sw1.*sw2;
             cond(1,n) = sum(swT(:));      
         ...
         xyDisp(aI,:) = [rDisp(ind,1),cDisp(ind,1),rDisp(ind,2),cDisp(ind,2)];
...
%       Set image Dimensions so they are identical.
        ...
        maxDisp = [max(xyDisp(aI,[1,3])),max(xyDisp(aI,[2,4]))];
        ...
        arV = (alDim(1):auDim(1)) + xyDisp(aI,1);
        acV = (alDim(2):auDim(2)) + xyDisp(aI,2);
        brV = (blDim(1):buDim(1)) + xyDisp(aI,3);
        bcV = (blDim(2):buDim(2)) + xyDisp(aI,4);
               
        A = zeros(maxdim + maxDisp);
        B = zeros(maxdim + maxDisp);
        aLbl = zeros(maxdim + maxDisp);
        bLbl = zeros(maxdim + maxDisp);
...
% Check for douplacates in the objectNext column array and remove them
    ...
    % address Object Track List.  
    if aI == 2
        objectTrack(:,aI-1)= 1:size(objectTrack,1);
    end
...
%             initTvTLag = oD.Tracks2.ObjectInfo.FitDataGompDT(n,1:4);
            ...
            oD.Tracks2.ObjectInfo.FitDataGompDT(n,1:end-1) = gompertzFitDT(tData, areaData, []);
...
elseif size(initVals,1)>1
    [estB, estTlag,estDT,  fssq1] = findPrmsGompBDt(initVals(:,2)',  initVals(:,3)', initVals(:,4)', tdata, idata);
    ...
    yn = gompBDt(prms, tdata);
    
    Tplat = nansum(cumsum(idata-yn));
...
if isempty(monData.IndexFile)
    ...
    ImageVars.Magnification = mP.mag;
    ImageVars.PixSize = mP.pixSize;
